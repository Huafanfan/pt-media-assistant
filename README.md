...
文档入口：[文档导航](docs/README.md)（现行说明、待审阅功能与历史记录）。
...
- **AI 推荐（可选启用）**：用自然语言描述类型、氛围和版本要求，多轮调整偏好；模型查证作品后查询 PT，服务端排序片源，最终仍由用户确认下载。配置和数据边界见 [AI 运行说明](docs/operations/AI_OPERATIONS.md)。
...
也可以从“AI 推荐”开始描述需求，推荐作品仍进入同一套详情和确认流程。方案依据和验收合同见 [AI 调研](docs/archive/ai/AI_RECOMMENDATION_RESEARCH.md) 与 [AI-001](docs/archive/ai/AI_RECOMMENDATION.md)。
