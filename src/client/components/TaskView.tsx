    ...
    return (
        <li className={`task-row is-${kind}`}>
            <div className="task-row-heading">
                <h3>{torrent.name}</h3>
                <span className={`task-state is-${kind}`}>
            ...
            <div className="task-actions">
                {confirming ? (
                    <>
                        <span className="task-confirm-note">
                        ...
                        <button
                            ...
                            onClick={() =>
                                void onAction("remove", [torrent.hash]).then(
                                    (ok) => {
                                        if (ok) onCancelRemove();
                                    },
                                )
                            }
                        >
                    ...
                    <>
                        <button
                            ...
                            disabled={actionPending}
                            onClick={() =>
                                void onAction(paused ? "resume" : "pause", [
                                    torrent.hash,
                                ])
                            }
                        >
...
export function TaskView({
    ...
    const visible = (
        filter === "all"
            ? torrents
            : torrents.filter((torrent) => classify(torrent) === filter)
    )
        .slice()
        .sort(
            (left, right) =>
                ...
                <div>
                    ...
                    <p>
                        {counts.downloading > 0
                            ? `${counts.downloading} 个进行中 · 共 ${counts.all} 个任务`
                            : `共 ${counts.all} 个任务`}
                    </p>
                </div>
                <button
                    ...
                    {seenItems.length > 0 ? (
                        <ul className="seen-list">
                            {seenItems.map((entry) => {
                                ...
                                return (
                                    <li key={key}>
                                        <span className="seen-title">
                                            {entry.title}
                                        </span>
                                        <span className="seen-meta">
                                            {entry.mediaType === "tv"
                                                ? "剧集"
                                                : "电影"}
                                        </span>
