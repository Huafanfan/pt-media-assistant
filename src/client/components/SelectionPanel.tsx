...
export function StorageMeter({
      ...
      const usedRatio =
            storage && storage.totalBytes > 0
                  ? Math.max(
                          0,
                          Math.min(1, storage.usedBytes / storage.totalBytes),
                    )
                  : 0;
                  ...
                  {storage ? (
                        <>
                              ...
                              <div className="storage-figures">
                                    <span>
                                          已用 {formatBytes(storage.usedBytes)}
                                    </span>
                                    <strong>
                                          剩余 {formatBytes(storage.freeBytes)}
                                    </strong>
                              </div>
      ...
      return (
            <li className="torrent-row">
                  ...
                  <div className="torrent-meta">
                        ...
                        <span>
                              {formatBytes(torrent.size * progress)} /{" "}
...
export function TorrentList({
      ...
      return (
            <section
                  ...
                  aria-labelledby="torrent-status-title"
            >
                  <div className="runtime-section-heading">
                        <div>
                  ...
                  {loading && active.length === 0 ? (
                        <p className="muted-line">正在读取任务状态…</p>
                  ) : null}
                  {!loading && !error && active.length === 0 ? (
                        <p className="muted-line">暂无进行中的下载。</p>
                  ) : null}
                  {active.length > 0 ? (
...
export function SelectionPanel({
      ...
      const indexLabel = selection
            ? String(selection.index).padStart(2, "0")
            : "";
      ...
      return (
            <aside
                  className={`selection-inspector ${expanded ? "is-expanded" : "is-collapsed"}`}
            ...
            >
                  ...
                  <div className="inspector-scroll">
                        {selection ? (
                              ...
                              >
                                    {release ? (
                                          <p className="selection-release-title">
                                                {release.title}
                                          </p>
                                    ) : null}
                                    {!preview && !previewError ? (
                                          <p className="muted-line selection-loading">
                                                正在检查片源与 NAS…
                                          </p>
                                    ) : null}
                                    {previewError ? (
                                    ...
                                    {preview ? (
                                          <>
                                                ...
                                                <div className="selection-actions">
                                                      ...
                                                      <button
                                                            className="primary-button grab-button"
                                                            type="button"
                                                            onClick={onConfirm}
                                                            disabled={
                                                                  !canConfirm
                                    ...
                                    {grabResult ? (
                                          <div
                                                className={`grab-result ${grabResult.accepted ? "is-accepted" : "is-rejected"}`}
                                                role="status"
                                          >
                                                <CheckCircle2
                                                      size={17}
