...
function responseError(response: Response, payload: unknown): ApiError {
  ...
  return new ApiError(
    cleanMessage(message ?? `请求失败（${response.status}）。`),
    response.status,
    code,
  );
}
...
async function requestAssistantTurnStream(
  ...
  const consumeLine = (line: string): void => {
    ...
    if (parsed.data.type === "error") {
      throw new ApiError(
        cleanMessage(parsed.data.error),
        502,
        parsed.data.code,
      );
    }
    if (
      parsed.data.data.clientTurnId !== request.clientTurnId ||
      (request.conversationId &&
        parsed.data.data.conversationId !== request.conversationId) ||
      (finalSnapshot &&
        (parsed.data.data.turnId !== finalSnapshot.turnId ||
          parsed.data.data.conversationId !== finalSnapshot.conversationId))
    ) {
      throw assistantStreamResponseError();
    }
...
function normalizeRelease(payload: unknown): ReleaseSummary | null {
  ...
  return {
    ...
    ...(value.freeleechState === "yes" ||
    value.freeleechState === "no" ||
    value.freeleechState === "unknown"
    ...
    ...(evidence
      ? {
          evidence: {
            resolution:
              evidence.resolution === "upstream" ||
              evidence.resolution === "title_inferred"
                ? evidence.resolution
                : "unknown",
            codec:
              evidence.codec === "upstream" ||
              evidence.codec === "title_inferred"
                ? evidence.codec
                : "unknown",
            size: evidence.size === "upstream" ? "upstream" : "unknown",
            seeders: evidence.seeders === "upstream" ? "upstream" : "unknown",
            season:
              evidence.season === "title_inferred"
                ? "title_inferred"
                : "unknown",
          },
        }
...
function normalizeSearch(payload: unknown): SearchResponse {
  ...
  return {
    ...
    intent: {
      ...
      ...(intent?.resolution === "2160p" ||
      intent?.resolution === "1080p" ||
      intent?.resolution === "720p"
        ? { resolution: intent.resolution }
        : {}),
      ...(typeof intent?.maxSizeBytes === "number"
        ? { maxSizeBytes: intent.maxSizeBytes }
        : {}),
      ...(typeof intent?.freeleechOnly === "boolean"
        ? { freeleechOnly: intent.freeleechOnly }
        : {}),
    },
...
function normalizeDiscoveryItem(payload: unknown): DiscoveryItem | null {
  ...
  const value = asRecord(payload);
  if (!media || typeof value?.rank !== "number" || !Number.isFinite(value.rank))
    return null;
  return {
    ...media,
...
function isSafeDiscoveryPosterUrl(
  value: unknown,
  id: string,
  mediaType: "movie" | "tv",
): value is string {
  ...
  if (
    /^\/api\/discovery\/collections\/(?:movie-hot|movie-weekly|tv-hot|tv-weekly|top250)\/items\/\d{1,16}\/poster\?page=\d+&limit=\d+$/u.test(
      value,
    )
  )
    return true;
  return /^\/api\/discovery\/actors\/\d{1,16}\/works\/\d{1,16}\/poster$/u.test(
    value,
  );
}
...
function normalizeDiscoveryMedia(payload: unknown): DiscoveryMedia | null {
  ...
  if (
    ...
    !title ||
    !mediaType ||
    !/^https:\/\/movie\.douban\.com\/subject\/\d+\/$/u.test(sourceUrl)
  )
    return null;
  ...
  const posterUrl = isSafeDiscoveryPosterUrl(value.posterUrl, id, mediaType)
    ? value.posterUrl
    : undefined;
  return {
    id,
    ...
    ...(typeof value.year === "string" && value.year.trim()
      ? { year: value.year.trim().slice(0, 12) }
      : {}),
    ...(rating >= 0 && rating <= 10 ? { rating } : {}),
    ...(typeof value.ratingCount === "number" &&
    Number.isFinite(value.ratingCount)
    ...
    ...(typeof value.role === "string" && value.role.trim()
      ? { role: value.role.trim().slice(0, 80) }
      : {}),
    sourceUrl,
  };
}
...
function normalizeDiscoveryActors(payload: unknown): DiscoveryActor[] {
  ...
  for (const entry of payload) {
    ...
    const name = (typeof entry === "string" ? entry : asString(value?.name))
      .trim()
      .slice(0, 80);
...
function normalizeDiscoveryActorProfile(
  payload: unknown,
): DiscoveryActorProfile {
  const value = asRecord(payload);
  const id = asString(value?.id);
  ...
  const works =
    normalizedId && Array.isArray(value?.works)
      ? value.works
          .map(normalizeDiscoveryMedia)
          .filter((work): work is DiscoveryMedia => work !== null)
      : [];
  const avatarUrl =
  ...
  const page = Math.min(
    100,
    Math.max(1, Math.floor(asFiniteNumber(value?.page, 1))),
  );
  const pageSize = Math.min(
    20,
    Math.max(1, Math.floor(asFiniteNumber(value?.pageSize, 10))),
  );
  const total = Math.max(
    0,
    Math.floor(asFiniteNumber(value?.total, works.length)),
  );
  return {
    id: normalizedId,
    name: name.slice(0, 120),
    ...(typeof value?.latinName === "string" && value.latinName.trim()
      ? { latinName: value.latinName.trim().slice(0, 160) }
      : {}),
    ...(avatarUrl ? { avatarUrl } : {}),
...
function normalizeDiscoveryMediaSearch(
  payload: unknown,
): DiscoveryMediaSearchResponse {
  const value = asRecord(payload);
  const items = Array.isArray(value?.items)
    ? value.items
        .map(normalizeDiscoveryMedia)
        .filter((item): item is DiscoveryMedia => item !== null)
    : [];
  return {
    query: asString(value?.query),
...
function normalizeDiscoveryCollection(
  ...
  const collection = discoveryCollections.has(rawCollection)
    ? rawCollection
    : expectedCollection;
  const items = Array.isArray(value?.items)
    ? value.items
  ...
  const total = Math.max(
    0,
    Math.floor(asFiniteNumber(value?.total, items.length)),
  );
  return {
    collection,
...
function normalizeDiscoveryReleases(
  payload: unknown,
): DiscoveryReleaseResponse {
  const value = asRecord(payload);
  const releases = Array.isArray(value?.releases)
    ...
    : [];
  const status =
    value?.status === "available" || value?.status === "possible"
      ? value.status
      : "unavailable";
  return {
    itemId: asString(value?.itemId),
    ...
    total: Math.max(
      0,
      Math.floor(asFiniteNumber(value?.total, releases.length)),
    ),
    releases,
  };
}
...
function normalizeTorrents(payload: unknown): TorrentSummary[] {
  ...
  const list = Array.isArray(payload)
    ? payload
    : value && Array.isArray(value.torrents)
      ? value.torrents
      : [];
  return list
    .map(normalizeTorrent)
    .filter((item): item is TorrentSummary => item !== null);
}
...
function normalizeStorage(payload: unknown): NasStorageSummary {
  ...
  const freeBytes = Math.min(
    totalBytes,
    Math.max(0, asFiniteNumber(value?.freeBytes)),
  );
  const usedBytes = Math.min(
    totalBytes,
    Math.max(0, asFiniteNumber(value?.usedBytes, totalBytes - freeBytes)),
  );
  return {
...
function discoveryQuery(page?: number, limit?: number): string {
  const values: string[] = [];
  if (page !== undefined)
    values.push(`page=${Math.min(100, Math.max(1, Math.floor(page)))}`);
  if (limit !== undefined)
    values.push(`limit=${Math.min(20, Math.max(1, Math.floor(limit)))}`);
  return values.length > 0 ? `?${values.join("&")}` : "";
}

function discoveryActorQuery(
  name: string,
  page?: number,
  limit?: number,
): string {
...
export function createApiClient(fetchImpl?: typeof fetch): ApiClient {
  const withCsrf = (csrfToken: string): HeadersInit => ({
    "X-CSRF-Token": csrfToken,
  });
  ...
  return {
    async getSession() {
      return normalizeSession(
        await requestJson<unknown>("/api/session", undefined, fetchImpl),
      );
    },

    async getHealth() {
      return normalizeHealth(
        await requestJson<unknown>("/api/health", undefined, fetchImpl),
      );
    },
    ...
    async getDiscoveryCollection(collection, csrfToken, page, limit) {
      return normalizeDiscoveryCollection(
        await requestJson<unknown>(
          `/api/discovery/collections/${encodeURIComponent(collection)}/items${discoveryQuery(page, limit)}`,
          {
            headers: withCsrf(csrfToken),
          },
          fetchImpl,
    ...
    async createAssistantTurnStream(request, csrfToken, onSnapshot, signal) {
      return requestAssistantTurnStream(
        request,
        csrfToken,
        onSnapshot,
        signal,
    ...
    async cancelAssistantTurn(turnId, csrfToken) {
      await requestJson<unknown>(
        `/api/assistant/turns/${encodeURIComponent(turnId)}/cancel`,
        {
          method: "POST",
          headers: withCsrf(csrfToken),
        },
