...
function assistantCardReleases(
...
function assistantReleaseResponseExpired(
  response: DiscoveryReleaseResponse | null | undefined,
): boolean {
...
function assistantDownloadIntent(message: string): boolean {
  return /(?:下载|加入下载|抓取)|(?:帮我|我要|我想).{0,12}(?:下|抓)/u.test(
    message,
  );
}
...
export function assistantReferenceIndex(message: string): number | null {
  const match = message.match(
    /第\s*([0-9]+|[一二三四五六七八九十]+)\s*(?:部|个版本?|项)?/u,
  );
...
export function App({ client = apiClient }: { client?: ApiClient } = {}) {
  ...
  const [sessionOverride, setSessionOverride] =
    useState<typeof bootstrap.session>(null);
  ...
  const [searchSubMode, setSearchSubMode] = useState<"media" | "release">(
    "media",
  );
  const [releaseResults, setReleaseResults] = useState<SearchResponse | null>(
    null,
  );
  const [releaseSearchError, setReleaseSearchError] = useState<string | null>(
    null,
  );
  ...
  const [assistantSelectedCard, setAssistantSelectedCard] =
    useState<AssistantRecommendationCard | null>(null);
  const [assistantReleaseResponses, setAssistantReleaseResponses] = useState<
    Record<string, DiscoveryReleaseResponse>
  >({});
  const [assistantReleaseLoading, setAssistantReleaseLoading] = useState(false);
  const [assistantReleaseError, setAssistantReleaseError] = useState<
    string | null
  >(null);
  const assistantRefreshRevision = useRef(0);
  const assistantStreamCardRef = useRef<AssistantRecommendationCard | null>(
    null,
  );
  useEffect(() => {
    if (
      !assistantSelectedCard ||
      assistantReleaseLoading ||
      selection ||
      confirmLoading
    )
      return;
    const latest = assistant.cards.find(
      (card) => card.cardId === assistantSelectedCard.cardId,
    );
    ...
    const followsStream =
      ...
      (previous.snapshotId
        ...
        : previous.checkedAt ===
          (priorStreamCard?.checkedAt ?? new Date(0).toISOString()));
    ...
    if (followsStream) {
      ...
      setAssistantReleaseResponses((current) => ({
        ...current,
        [latest.cardId]: assistantCardReleases(latest),
      }));
      if (
        latest.availability !== "unchecked" &&
        latest.availability !== "error"
      )
  ...
  const [selectedMediaItem, setSelectedMediaItem] =
    useState<DiscoveryMedia | null>(null);
  const [mediaOrigin, setMediaOrigin] = useState<MediaOrigin | null>(null);
  const [selectedActorName, setSelectedActorName] = useState<string | null>(
    null,
  );
  const [actorHistory, setActorHistory] = useState<ActorNavigationEntry[]>([]);
  const [discoveryInspectorError, setDiscoveryInspectorError] = useState<
    string | null
  >(null);
  const [discoveryDetailsError, setDiscoveryDetailsError] = useState<
    string | null
  >(null);
  const actor = useDiscoveryActor(
    client,
    csrfToken,
    selectedActorName,
    paired && Boolean(selectedActorName),
  );
  const standaloneMedia = useMediaInspector(
    ...
    selectedMediaItem,
    Boolean(
      selectedMediaItem && mediaOrigin && mediaOrigin.kind !== "collection",
    ),
  ...
  const seenKeys = useMemo(
    () =>
      new Set(
        historySeen.map((entry) => `${entry.mediaType}:${entry.mediaId}`),
      ),
    [historySeen],
  );

  const toggleSeen = async (media: DiscoveryMedia): Promise<void> => {
    if (!csrfToken || !client.markSeen || !client.unmarkSeen || seenPendingKey)
      return;
    ...
    try {
      if (seenKeys.has(key)) {
        ...
        await client.markSeen(
          { mediaId: media.id, mediaType: media.mediaType, title: media.title },
          csrfToken,
        );
        setHistorySeen((current) => [
          {
  ...
  const unmarkSeenEntry = async (entry: SeenMediaEntry): Promise<void> => {
    ...
    try {
      ...
      setHistorySeen((current) =>
        current.filter((item) => `${item.mediaType}:${item.mediaId}` !== key),
      );
    } catch (error) {
      setHistoryError(readableError(error));
    } finally {
  ...
  const handleTorrentAction = async (
  ...
  const hasInspector = Boolean(
    selection || selectedMediaItem || selectedActorName,
  );

  const sessionFailure =
    !bootstrap.loading && !session && bootstrap.sessionError;
      ...
      if (releaseMode) {
        ...
        setMessages((current) => [
          ...current,
          {
            id: messageId(),
            role: "assistant",
            text: `找到 ${response.total} 个片源，选择后仍需确认才会加入下载。`,
            createdAt: Date.now(),
          },
        ]);
      } else {
        const response = await client.searchDiscoveryMedia(
          nextQuery,
          csrfToken,
          10,
        );
        ...
        setMessages((current) => [
          ...current,
          {
            id: messageId(),
            role: "assistant",
            text: initialAssistantMessage(response.total),
            createdAt: Date.now(),
          },
        ]);
      }
  ...
  const handleAssistantCard = (card: AssistantRecommendationCard) => {
    if (confirmLoading || card.identityStatus === "unverified") {
      if (card.identityStatus === "unverified") {
        setAssistantReleaseError(
          "这部作品的身份尚未核实，暂不能查看详情或下载。",
        );
      }
      return;
    }
    ...
    setAssistantReleaseError(
      assistantReleaseResponseExpired(snapshot)
  ...
  const handleSelect = async (release: ReleaseSummary, index: number) => {
    if (mediaOrigin?.kind === "assistant") {
      ...
      if (assistantReleaseResponseExpired(snapshot)) {
        setAssistantReleaseError("片源引用已过期，请刷新后重新选择。");
        return;
      }
    }
  ...
  const handleMediaItem = (item: DiscoveryMedia, origin: MediaOrigin) => {
    ...
    if (origin.kind === "collection") {
      void Promise.all([
        discovery.ensureAvailability(item),
        discovery.ensureDetails(item),
      ]).then(([availability, details]) => {
        if (!availability)
          setDiscoveryInspectorError("这次片源检查没有完成，请稍后重试。");
        if (!details)
          setDiscoveryDetailsError("演职员信息暂时不可用，可以稍后重试。");
      });
    }
  };
    ...
    if (mediaOrigin.kind === "assistant") {
      ...
      void client
        ...
        .then((response) => {
          ...
          setAssistantSelectedCard((current) =>
            current?.cardId === cardId
              ? {
                  ...
                  ...(response.expiresAt
                    ...
                    : {}),
                  ...(response.actionableUntil
                    ? { actionableUntil: response.actionableUntil }
                    : {}),
                }
              : current,
          );
        ...
        .finally(() => {
          if (assistantRefreshRevision.current === revision)
            setAssistantReleaseLoading(false);
        });
      return;
    }
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const result = await client.grab(
        { releaseId: preview.release.id, confirm: true },
        csrfToken,
      );
  ...
  const collectionOrigin =
    mediaOrigin?.kind === "collection" ? mediaOrigin : null;
  const selectedDetails =
    selectedMediaItem && collectionOrigin
      ? (discovery.detailsById[selectedMediaItem.id] ?? null)
      : standaloneMedia.details;
  const selectedDetailsLoading = Boolean(
    selectedMediaItem && collectionOrigin
      ? discovery.detailsLoadingIds.has(selectedMediaItem.id)
      : standaloneMedia.detailsLoading,
  );
  const selectedDetailsError = collectionOrigin
    ? discoveryDetailsError
    : standaloneMedia.detailsError;
  const selectedReleases =
    mediaOrigin?.kind === "assistant"
      ? (assistantReleaseResponses[mediaOrigin.cardId] ?? null)
      : selectedMediaItem && collectionOrigin
        ? (discovery.availabilityById[selectedMediaItem.id] ?? null)
        : standaloneMedia.releaseResponse;
  const selectedReleaseLoading =
    mediaOrigin?.kind === "assistant"
      ? assistantReleaseLoading
      : Boolean(
          selectedMediaItem && collectionOrigin
  ...
  const selectedReleaseError =
    mediaOrigin?.kind === "assistant"
      ? assistantReleaseError
      : collectionOrigin
        ? discoveryInspectorError
        : standaloneMedia.releaseError;
    ...
    <div className="discovery-inspector-stack">
      <MediaInspector
        ...
        onSelectRelease={(release) => {
          const releaseIndex =
            selectedReleases?.releases.findIndex(
        ...
        seen={seenKeys.has(
          `${selectedMediaItem.mediaType}:${selectedMediaItem.id}`,
        )}
        seenPending={
          seenPendingKey ===
          `${selectedMediaItem.mediaType}:${selectedMediaItem.id}`
    ...
    return (
      <div className="app-shell">
        <AppHeader
          ...
          paired={false}
          healthError={bootstrap.healthError}
        />
    ...
    return (
      <div className="app-shell">
        <AppHeader
          ...
          paired={false}
          healthError={bootstrap.healthError}
          onRefresh={bootstrap.reload}
        />
        <main className="boot-state">
          <OfflineState
            detail={bootstrap.sessionError ?? "请确认片源服务正在运行。"}
          />
          <button
            className="outline-button retry-button"
            type="button"
  ...
  return (
    <div className="app-shell">
      ...
      />
      <main
        className={`app-layout ${mode === "discover" ? "is-discovery" : mode === "tasks" ? "is-tasks" : "search-shell"} ${hasInspector ? "has-inspector" : "no-inspector"}`}
      >
        {mode === "discover" ? (
          <>
          ...
          <>
            <section
              className="chat-pane assistant-pane"
              aria-label="AI 推荐对话"
            >
              <div className="search-mode-toolbar">
                <ModeSwitch mode={mode} onChange={handleModeChange} />
            ...
            <aside className="results-pane" aria-label="推荐作品">
              <div className="results-scroll">
                <AssistantRecommendations
                  cards={assistant.cards}
                  pendingRecommendations={assistant.pendingRecommendations}
                  phase={assistant.phase}
                  snapshots={assistantReleaseResponses}
                  selectedCardId={assistantSelectedCard?.cardId ?? null}
                  onSelect={handleAssistantCard}
                  onFallback={() => handleModeChange("search")}
            ...
            <div className="composer-dock">
              <QueryComposer
                ...
                placeholder="例如：轻松的科幻电影，1080p，15GB 以内"
                inputLabel="描述想看的类型和要求"
              />
            </div>
          </>
        ) : (
          <>
              ...
              <div className="search-mode-toolbar">
                ...
                >
                  <button
                    ...
                    className={
                      searchSubMode === "media" ? "is-active" : undefined
                    }
                  ...
                  <button
                    ...
                    className={
                      searchSubMode === "release" ? "is-active" : undefined
                    }
            ...
            <aside
              ...
              aria-label={
                searchSubMode === "release" ? "片源直搜结果" : "作品搜索结果"
              }
            >
              <div className="results-scroll">
                {searchSubMode === "release" ? (
                  <>
                    {releaseResults ? (
                      <div
                        className="release-search-intent"
                        aria-label="解析后的搜索条件"
                      >
                        <span>关键词 {releaseResults.intent.searchTerm}</span>
                        {releaseResults.intent.resolution ? (
                          <span>{releaseResults.intent.resolution}</span>
                        ) : null}
                        {releaseResults.intent.maxSizeBytes ? (
                          <span>
                            ≤ {formatBytes(releaseResults.intent.maxSizeBytes)}
                          </span>
                        ) : null}
                        {releaseResults.intent.freeleechOnly ? (
                          <span>仅免费</span>
                        ) : null}
                        <span className="release-search-elapsed">
                          {releaseResults.elapsedMs} ms
                        </span>
                      </div>
                    ) : null}
                    {searchState === "loading" ? (
                      <LoadingState label="正在查询片源…" />
                    ) : null}
                    {releaseSearchError ? (
                  ...
                  <MediaSearchResults
                    ...
                    selectedItemId={
                      mediaOrigin?.kind === "search"
                        ? (selectedMediaItem?.id ?? null)
                        : null
                    }
                    onSelect={(item) =>
                      handleMediaItem(item, {
            ...
            <div className="composer-dock">
              <QueryComposer
                ...
                {...(searchSubMode === "release"
                  ? {
                      placeholder: "输入片名、1080p、10GB 以内等条件",
                      inputLabel: "输入片源搜索条件",
                    }
                  : {})}
              />
