...
import { assistantPreferencesSchema, assistantPreferencesPatchSchema, type AssistantPreferences, type AssistantWarning } from '../../shared/assistant.js';
...
export type AssistantDiscovery = Pick<DiscoveryService, 'searchMedia' | 'getMedia' | 'getMediaDetails' | 'getMediaReleases'>;
...
export const toolDefinitions: AssistantToolDefinition[] = Object.entries(schemas).map(([name,schema]) => ({type:'function',function:{ name,
  description: name === 'resolve_media' ? '按具体片名核实公开作品，返回类型/简介。首次调用同时提交本轮偏好变更，最多5个片名。' : name === 'get_media_details' ? '取得已知作品的演员和导演。' : name === 'check_pt_availability' ? '对已知作品缓存优先只读检查PT（每轮最多3部）。允许偏好更新。' : '用服务端规则对已有片源快照排序，允许更新偏好，不增加PT请求。',
...
export class ToolRunner {
  executions = 0; searches = 0; ptRequests = 0;
  checked = new Set<string>(); touched = new Set<string>();
  warnings: AssistantWarning[] = [];
  constructor(readonly discovery: AssistantDiscovery, readonly conversation: Conversation, readonly signal: AbortSignal) {}
  async execute(name: string, raw: unknown): Promise<unknown> {
    ...
    if ('preferences' in args) updatePreferences(this.conversation, args.preferences);
    if ('query' in args) {
    ...
    if (name === 'get_media_details') {
      const details = await this.discovery.getMediaDetails(args.mediaType,args.mediaId);
      return {mediaId:args.mediaId, actors:details.actors.slice(0,12).map(x=>x.name),directors:details.directors.slice(0,12)};
    }
    if (name === 'check_pt_availability') {
      ...
      if (this.checked.has(key)) {
        const cachedCard = buildRecommendationCard(this.conversation.id, 'tool_result_0000', 0, candidate, this.conversation.preferences);
        return {
      ...
      if (this.checked.size >= 3) throw new AssistantError('AI_BUDGET_EXCEEDED');
      this.checked.add(key);
      try {
        const snapshot = await this.discovery.getMediaReleases(args.mediaType,args.mediaId,10,{signal:this.signal,beforeSearch:()=>{
          this.signal.throwIfAborted(); if (++this.ptRequests > 6) throw new AssistantError('AI_BUDGET_EXCEEDED');
        }});
        ...
        this.warnings.push({code:'PT_UNAVAILABLE',message:'PT 查询失败，不代表没有资源。',mediaId:args.mediaId});
      }
    }
    const card = buildRecommendationCard(this.conversation.id,'tool_result_0000',0,candidate,this.conversation.preferences);
    return {mediaId:args.mediaId,availability:card.availability,checkedAt:card.checkedAt,evidenceIds:card.evidenceIds,constraintResults:card.constraintResults,
      // Never send raw indexer labels, URLs or raw Prowlarr objects to the model.
      releases: card.rankedReleases.map((r,index)=>({id:r.id,title:r.title,size:r.evidence?.size==='unknown'?null:r.size,seeders:r.evidence?.seeders==='unknown'?null:r.seeders,resolution:r.resolution,freeleechState:r.freeleechState,rank:index+1,reasonCodes:r.reasonCodes}))};
  }
}
