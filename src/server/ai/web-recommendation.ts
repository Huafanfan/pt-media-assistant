...
const nullablePreferenceKeys = new Set(['yearFrom', 'yearTo', 'resolution', 'maxSizeBytes']);
const modelPreferencesSchema = z.preprocess(value => {
  ...
  return Object.fromEntries(Object.entries(value)
    .filter(([key, entry]) => entry !== null || nullablePreferenceKeys.has(key))
    .map(([key, entry]) => {
      if (['yearFrom', 'yearTo', 'maxSizeBytes'].includes(key) && typeof entry === 'string' && /^\d+$/u.test(entry)) return [key, Number(entry)];
      if (key === 'resolution' && typeof entry === 'string') return [key, entry.toLowerCase() === '4k' ? '2160p' : entry.toLowerCase()];
      return [key, entry];
    }));
...
const outputSchema = z.object({
  ...
  recommendations: z.array(z.object({
    ...
    year: z.preprocess(value => typeof value === 'number' && Number.isInteger(value) ? String(value) : value,
      z.string().regex(/^\d{4}$/u).nullish().transform(value => value ?? undefined)),
...
const OFFICIAL_DOMAINS = ['mgtv.com', 'iqiyi.com', 'v.qq.com', 'youku.com', 'netflix.com', 'bbc.co.uk', 'channel4.com', 'tvn.cjenm.com', 'foodnetwork.com'];
export type AssistantStageMetric = { stage: 'search' | 'model' | 'metadata' | 'pt'; durationMs: number };
...
function normalize(value: string): string { return value.toLocaleLowerCase().replace(/[^\p{L}\p{N}]/gu, ''); }
function cleanReason(value: string): string {
  return /https?:|做种|免费资源|有片源|可下载|torrent|seeder/iu.test(value) ? '根据公开节目介绍匹配本轮需求。' : value;
}
function positiveRequest(user: string): string {
  return user.replace(/(?:不要|不看|排除|不想看|别推荐|不需要|不是)[^，。；,.!?！？;]*/gu, '');
}
function requestedType(user: string): 'tv' | 'movie' | undefined {
  const positive = positiveRequest(user);
  if (/综艺|电视剧|剧集/u.test(positive) && /电影/u.test(positive) && /或|都可以|不限/u.test(positive)) return undefined;
...
function needsHardEvidence(card: AssistantRecommendationCard, prefs: AssistantPreferences): boolean {
  if ((prefs.yearFrom !== null || prefs.yearTo !== null) && !/^\d{4}$/u.test(card.year ?? '')) return true;
...
export class WebRecommendationService {
  ...
  async run(owner: string, request: AssistantTurnRequest, externalSignal?: AbortSignal,
    onSnapshot?: (response: AssistantTurnResponse) => void, onMetric?: (metric: AssistantStageMetric) => void): Promise<AssistantTurnResponse> {
    const { turn, cached } = this.store.start(owner, request.clientTurnId, request.conversationId, request.message);
    if (cached) { onSnapshot?.(turn.result!); return turn.result!; }
    ...
    const timer = setTimeout(() => turn.controller.abort('timeout'), this.options.timeoutMs ?? 60_000);
    ...
    try {
      const aborted = new Promise<never>((_, reject) => {
        rejectAbort = () => reject(new AssistantError(turn.controller.signal.reason === 'timeout' ? 'AI_TIMEOUT' : 'AI_CANCELLED'));
        turn.controller.signal.addEventListener('abort', rejectAbort, { once: true });
        if (turn.controller.signal.aborted) rejectAbort();
      });
      const result = await Promise.race([this.execute(turn, request.message, onSnapshot ? response => {
        onSnapshot(response);
        published = response;
  ...
  private async execute(turn: Turn, user: string, onSnapshot?: (response: AssistantTurnResponse) => void, onMetric?: (metric: AssistantStageMetric) => void): Promise<AssistantTurnResponse> {
    const signal = turn.controller.signal, c = turn.conversation;
    const measure = async <T>(stage: AssistantStageMetric['stage'], operation: () => Promise<T>): Promise<T> => {
      const started = Date.now();
      try { return await operation(); } finally { onMetric?.({ stage, durationMs: Date.now() - started }); }
    };
    const warnings: AssistantWarning[] = [];
    const usage = { promptTokens: 0 as number | null, completionTokens: 0 as number | null, totalTokens: 0 as number | null, modelRequests: 0, toolExecutions: 0 };
    ...
    const snapshot = (phase: 'verifying' | 'checking' | 'complete') => {
      ...
      const response = assistantTurnResponseSchema.parse({ conversationId: c.id, turnId: turn.id, clientTurnId: turn.clientTurnId,
        text: recommendations.length ? `推荐这 ${recommendations.length} 部。` : pending.length ? c.preferences.onlyAvailable ? '已找到相关作品，尚未确认可用片源。' : '已找到相关作品，部分条件尚未核实。' : warnings.some(w => w.code === 'WEB_SEARCH_UNAVAILABLE') ? '联网搜索暂时不可用，请稍后重试。' : '这次未找到有足够来源依据的推荐。',
        preferences: c.preferences, recommendations, ...(pending.length ? { pendingRecommendations: pending } : {}), warnings: warnings.slice(0, 20), usage, phase });
      onSnapshot?.(response);
      return response;
    };
    // Search receives only the public topic, never credentials, history or PT URLs.
    const publicQuery = user.replace(/https?:\/\/\S+/gu, '').replace(/(?:passkey|token|api[_-]?key)\s*[:=]\s*\S+/giu, '').slice(0, 300).trim();
    if (!publicQuery) throw new AssistantError('AI_INVALID_OUTPUT');
    const last = c.history.at(-1);
    const refinement = /继续|换一|看过|刚才|这些|这部|那部|第[一二三四五六七八九十\d]+部|^(?:不要|只要|最好|改成|加上)/u.test(user)
      && !(requestedType(user) && requestedType(user) !== c.preferences.mediaType);
    const contextHint = refinement && last ? (c.topic ?? last.user).replace(/https?:\/\/\S+/gu, '').slice(0, 180) : '';
    const date = new Date(this.store.now());
    const recentHint = /最近|今年|最新|新播/u.test(user) ? String(date.getUTCFullYear()) : '';
    const topicHint = /做饭|做菜|烹饪|厨艺/u.test(user) ? '烹饪过程 厨师 做菜' : '';
    const queries = [...new Set([`${publicQuery} ${recentHint} ${topicHint} ${contextHint} 节目 介绍`, `${publicQuery} ${recentHint} ${topicHint} ${contextHint} 官方 推荐`].map(q => q.trim().slice(0, 400)))];
    const responses = await measure('search', () => Promise.all(queries.map((query, index) => this.search.search(query, { signal, ...(index === 0 ? { includeDomains: OFFICIAL_DOMAINS } : {}) }))));
    signal.throwIfAborted();
    usage.toolExecutions += queries.length;
    if (responses.some(r => r.status === 'unavailable')) warnings.push({ code: 'WEB_SEARCH_UNAVAILABLE', message: '部分搜索来源暂时不可用。' });
    const sourceMap = new Map<string, WebSearchResult>();
    for (const source of responses.flatMap(r => r.results)) if (!sourceMap.has(source.id)) sourceMap.set(source.id, source);
    const sources = [...sourceMap.values()].slice(0, 8);
    if (!sources.length) return snapshot('complete');
    const context = { user, currentDate: date.toISOString().slice(0, 10), previousPreferences: c.preferences, knownMedia: [...c.candidates.values()].slice(-15).map(x => ({ id: x.media.id, title: x.media.title })),
      lastRecommendations: last ? [...last.response.recommendations, ...(last.response.pendingRecommendations ?? [])].map(x => ({ id: x.mediaId, title: x.title })) : [],
      sources: sources.map((x, index) => ({ id: `s${index + 1}`, title: x.title, content: x.content.slice(0, 450) })) };
    const result = await measure('model', () => this.provider.chat([{ role: 'system', content: SYSTEM }, { role: 'user', content: JSON.stringify(context) }], [], { signal, maxTokens: 1200 }));
    signal.throwIfAborted();
    usage.modelRequests = 1;
    usage.promptTokens = result.usage.promptTokens; usage.completionTokens = result.usage.completionTokens; usage.totalTokens = result.usage.totalTokens;
    let output: z.infer<typeof outputSchema>;
    try { output = outputSchema.parse(JSON.parse((result.message.content ?? '').replace(/^```(?:json)?\s*/u, '').replace(/\s*```$/u, ''))); }
    catch (error) {
      const failure = new AssistantError('AI_INVALID_OUTPUT');
      failure.cause = new Error(result.finishReason === 'length' ? 'MODEL_OUTPUT_TRUNCATED' : error instanceof z.ZodError ? `MODEL_SCHEMA_INVALID:${error.issues.slice(0, 8).map(issue => `${issue.path.join('.')}:${issue.code}`).join(',')}` : 'MODEL_JSON_INVALID');
      throw failure;
    }
    const explicitType = requestedType(user);
    if (output.scope === 'new' || (explicitType && explicitType !== c.preferences.mediaType)) {
      c.preferences = { ...defaultAssistantPreferences(), seenMediaIds: c.preferences.seenMediaIds };
      c.topic = publicQuery;
    }
    c.topic ??= publicQuery;
    const sourceSeen = new Set(c.seenSourceTitles ?? []);
    const historyCards = c.history.flatMap(entry => [...entry.response.recommendations, ...(entry.response.pendingRecommendations ?? [])]);
    if (output.preferences.seenMediaIds) {
      output.preferences.seenMediaIds = [...new Set([...c.preferences.seenMediaIds, ...output.preferences.seenMediaIds.filter(id => {
        const prior = historyCards.find(card => card.mediaId === id && card.identityStatus === 'unverified');
    ...
    if (controls.resolution) output.preferences.resolution = controls.resolution;
    if (controls.maxSizeBytes) output.preferences.maxSizeBytes = controls.maxSizeBytes;
    if (/只要有资源|只看有资源/u.test(user)) output.preferences.onlyAvailable = true;
    if (/只要免费|必须免费|只看免费/u.test(user)) { output.preferences.freeleechRequired = true; output.preferences.freeleechPreferred = false; }
    else if (/最好免费|优先免费/u.test(user)) { output.preferences.freeleechPreferred = true; output.preferences.freeleechRequired = false; }
    updatePreferences(c, output.preferences);
    if (explicitType) c.preferences = { ...c.preferences, mediaType: explicitType };
    const positive = positiveRequest(user);
    const requestedVariety = /综艺/u.test(positive);
    const requestedDocumentary = /纪录片/u.test(positive);
    const eitherKind = requestedVariety && requestedDocumentary && /或|都可以|不限/u.test(positive);
    const excludesVariety = /(?:不要|不看|排除|不想看|别推荐|不需要)\s*(?:任何)?综艺/u.test(user);
    const excludesDocumentary = /(?:不要|不看|排除|不想看|别推荐|不需要)\s*(?:任何)?纪录片/u.test(user);
    const selected: Array<{ proposed: z.infer<typeof outputSchema>['recommendations'][number]; sources: WebSearchResult[] }> = [];
    const titles = new Set<string>();
    for (const candidate of c.candidates.values()) {
      if (c.preferences.seenMediaIds.includes(candidate.media.id) || c.preferences.seenMediaIds.includes(`${candidate.media.mediaType}:${candidate.media.id}`)) titles.add(normalize(candidate.media.title));
    }
    ...
    for (const proposed of output.recommendations) {
      ...
      const evidence = sources.filter((source, sourceIndex) => (proposed.sourceIds.includes(`s${sourceIndex + 1}`) || proposed.sourceIds.includes(source.id)) && (
        normalize(source.title).includes(name)
        || (quote.length >= 12 && quote.includes(name) && normalize(source.content).includes(quote) && !/相关推荐|相关阅读|下一集|上一集|导航/u.test(proposed.supportingQuote))
      ));
      if (!name || titles.has(name) || !evidence.length
        || (excludesVariety && proposed.contentKind === 'variety') || (excludesDocumentary && proposed.contentKind === 'documentary')
        || (!eitherKind && requestedVariety && proposed.contentKind !== 'variety') || (!eitherKind && requestedDocumentary && proposed.contentKind !== 'documentary')
        || (eitherKind && !['variety', 'documentary'].includes(proposed.contentKind))) continue;
      titles.add(name);
      selected.push({ proposed, sources: evidence });
      cards.push({ cardId: `web_${turn.id}_${cards.length}`, mediaId: `web_${evidence[0]!.id}`.slice(0, 64), mediaType: proposed.mediaType,
        title: proposed.title, ...(proposed.year ? { year: proposed.year } : {}), genres: [], summary: evidence[0]!.content.slice(0, 900), reason: cleanReason(proposed.reason),
        evidenceIds: evidence.map(x => `web:${x.id}`).slice(0, 20), constraintResults: [], availability: 'unchecked', rankedReleases: [], identityStatus: 'unverified',
        contentKind: proposed.contentKind, sources: evidence.map(({ id, title, url }) => ({ id, title, url })) });
    }
    snapshot('verifying');
    ...
    await Promise.all(selected.map(async ({ proposed }, index) => {
      try {
        ...
        const matches = found.items.filter(item => [item.title, item.originalTitle ?? ''].some(t => normalize(t) === normalize(proposed.title)) && (!proposed.year || item.year === proposed.year));
        if (matches.length !== 1) return;
        const media = await measure('metadata', () => this.discovery.getMedia(proposed.mediaType, matches[0]!.id));
        signal.throwIfAborted(); usage.toolExecutions++;
        if (media.id !== matches[0]!.id || media.mediaType !== proposed.mediaType || ![media.title, media.originalTitle ?? ''].some(t => normalize(t) === normalize(proposed.title)) || (proposed.year && media.year !== proposed.year)) return;
        ...
        cards[index] = { ...buildRecommendationCard(c.id, turn.id, index, c.candidates.get(key)!, c.preferences, cleanReason(proposed.reason)),
          cardId: previous.cardId, identityStatus: 'verified', sources: previous.sources, contentKind: proposed.contentKind };
        snapshot('checking');
      } catch {
        signal.throwIfAborted();
    ...
    while (c.candidates.size > 100) c.candidates.delete(c.candidates.keys().next().value!);
