...
function safeReason(text: string): string {
  ...
  return /(?:https?:|免费|free(?:leech)?|做种|seed(?:er|ing)?|peer|片源|资源|下载|download|torrent|字幕|subtitle|音轨|audio|杜比|dolby|HDR|codec|编码|分辨率|resolution|评分|rating|score|大小|size|可用|available|存在|found|\d)/iu.test(
    text,
  )
    ? ""
    : text.slice(0, 800);
...
export class AssistantService {
  ...
  async run(
    ...
    try {
      ...
      const result = await Promise.race([
        this.execute(turn, request.message),
        aborted,
      ]);
      turn.result = result;
      ...
      turn.failed = true;
      turn.conversation.preferences = previousPrefs;
      if (e instanceof AssistantError) throw e;
      if (e instanceof ProviderError)
        throw new AssistantError(
          e.code === "AI_RATE_LIMITED" ? "AI_UNAVAILABLE" : e.code,
          e.code === "AI_RATE_LIMITED" ? 429 : 503,
          e.retryAfterSeconds,
        );
  ...
  private async execute(
    ...
    const context = {
      currentDate: new Date(this.store.now()).toISOString().slice(0, 10),
      preferences: c.preferences,
      lastCards:
        recent
          .at(-1)
          ?.response.recommendations.map((x) => ({
            mediaId: x.mediaId,
    ...
    const messages: AssistantMessage[] = [
      ...
      ...recent.flatMap((x) => [
        { role: "user" as const, content: x.user.slice(0, 1000) },
        { role: "assistant" as const, content: x.response.text.slice(0, 500) },
      ]),
    ...
    while (
      messages.length > 3 &&
      Buffer.byteLength(
        JSON.stringify(messages) + JSON.stringify(toolDefinitions),
        "utf8",
      ) > 9_000
    )
      messages.splice(2, 2);
    ...
    for (let step = 0; step < 4; step++) {
      ...
      if (messageBytes > 12_000) {
        runner.warnings.push({
          code: "AI_BUDGET_EXCEEDED",
          message: "上下文达到上限，已保留查证结果。",
        });
        break;
      }
      const requestTools =
        ...
        messageBytes +
          ...
          ? []
          : toolDefinitions;
      const result = await this.provider.chat(messages, requestTools, {
        signal,
        maxTokens: 1200,
      });
      usage.modelRequests++;
      for (const k of [
        ...
        usage[k] =
          usage[k] === null || result.usage[k] === null
            ? null
            : usage[k]! + result.usage[k]!;
      signal.throwIfAborted();
      if (result.message.role !== "assistant")
        throw new AssistantError("AI_INVALID_OUTPUT");
      const calls = result.message.tool_calls;
      if (calls?.length) {
        if (!requestTools.length || calls.length > 8 - runner.executions) {
          runner.warnings.push({
            code: "AI_BUDGET_EXCEEDED",
            message: "已达到本轮查询上限。",
          });
        ...
        for (const call of calls) {
          ...
          try {
            ...
            if (e instanceof AssistantError) {
              ...
              value = {
                error: "INVALID_TOOL_ARGUMENTS_OR_UPSTREAM",
                message: "请检查合法工具、已知ID和偏好参数；最多修复一次。",
              };
            else {
              halt = true;
              runner.warnings.push({
                code: "AI_INVALID_OUTPUT",
                message: "部分查询未能完成，请重试或按片名搜索。",
              });
              value = { error: "TOOL_FAILED" };
            }
          }
          messages.push({
            role: "tool",
      ...
      try {
        ...
        messages.push(
          {
            role: "assistant",
            content: (result.message.content ?? "").slice(0, 4000),
          },
          {
            role: "user",
            content:
              "输出未通过JSON结构验证。请按要求返回合法JSON，不增加工具请求。",
          },
        );
      }
    }
    ...
    const keys = requested.length
      ? requested.flatMap((x) =>
          [...c.candidates.keys()]
            .filter((key) => c.candidates.get(key)!.media.id === x.mediaId)
            .slice(0, 1),
        )
      : [...runner.touched];
    ...
    const autoCheckKeys = [...new Set(keys)]
      .filter((key) => {
        ...
        const actionableUntil =
          candidate.snapshot?.actionableUntil ?? candidate.snapshot?.expiresAt;
    ...
    const checkCandidate = async (key: string): Promise<void> => {
      ...
      if (!candidate) return;
      try {
        await runner.execute("check_pt_availability", {
    ...
    for (const key of [...new Set(keys)].slice(0, 5)) {
      ...
      const card = buildRecommendationCard(
        ...
        candidate,
        c.preferences,
        model ? safeReason(model.reason) : "",
      );
      if (c.preferences.onlyAvailable && card.availability !== "available")
        continue;
    ...
    if (!output)
      warnings.push({
        code: "AI_INVALID_OUTPUT",
        message: "AI 回复未完整通过验证，以下仅显示已查证的结果。",
      });
    const text = recommendations.length
      ? `推荐这 ${recommendations.length} 部。`
      : output?.text &&
          !/(?:资源|下载|做种|免费|https?:|\d)/iu.test(output.text)
        ? output.text
        : "暂时没有符合条件的推荐，试试放宽条件。";
    return assistantTurnResponseSchema.parse({
      conversationId: c.id,
      turnId: turn.id,
