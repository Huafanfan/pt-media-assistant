...
export class ConversationStore {
  ...
  private cleanup() {
    ...
    for (const [id, turn] of this.turns)
      if (!turn.conversation.active && now - turn.createdAt >= 30 * 60_000) {
        this.turns.delete(id);
        this.clientIds.delete(`${turn.owner}:${turn.clientTurnId}`);
      }
    for (const [id, c] of this.conversations)
      if (now - c.touched >= 24 * 60 * 60_000) this.remove(c.owner, id);
  ...
  start(
    ...
    if (old) {
      ...
      if (
        ...
        (conversationId && conversationId !== old.conversation.id)
      )
        throw new AssistantError("TURN_IN_PROGRESS", 409);
      if (old.result) return { turn: old, cached: true };
      throw new AssistantError(
        old.failed ? "AI_CANCELLED" : "TURN_IN_PROGRESS",
        409,
      );
    }
    let c = conversationId ? this.conversations.get(conversationId) : undefined;
    if (conversationId && (!c || c.owner !== owner))
      throw new AssistantError("CONVERSATION_EXPIRED", 404);
    if (c?.active) throw new AssistantError("TURN_IN_PROGRESS", 409);
    const resuming = Boolean(c);
    if (
      this.starts.length >= 60 ||
      this.starts.filter((x) => x.owner === owner && this.now() - x.at < 60_000)
        .length >= 3
    )
      throw new AssistantError("AI_BUDGET_EXCEEDED", 429, 60);
    if ([...this.conversations.values()].filter((x) => x.active).length >= 2)
      throw new AssistantError("TURN_IN_PROGRESS", 429, 5);
    if (!c) {
      ...
      c = {
        ...
        owner,
        touched: this.now(),
        preferences: this.seed?.preferences() ?? defaultAssistantPreferences(),
        candidates: new Map(),
        history: [],
      };
    ...
    if (c.history.length >= 20)
      throw new AssistantError("CONVERSATION_EXPIRED", 409);
    const id = randomUUID();
    c.active = id;
    c.touched = this.now();
    const turn = {
    ...
    this.clientIds.set(`${owner}:${clientTurnId}`, id);
    this.starts.push({ owner, at: this.now() });
    return { turn, cached: false };
  }
  cancel(owner: string, id: string) {
    const turn =
      this.turns.get(id) ??
      this.turns.get(this.clientIds.get(`${owner}:${id}`) ?? "");
    if (!turn || turn.owner !== owner)
      throw new AssistantError("TURN_NOT_FOUND", 404);
  ...
  remove(owner: string, id: string) {
    ...
    for (const [turnId, turn] of this.turns)
      if (turn.conversation.id === id) {
        this.turns.delete(turnId);
        this.clientIds.delete(`${turn.owner}:${turn.clientTurnId}`);
      }
  }
  close() {
    for (const t of this.turns.values()) t.controller.abort();
    this.turns.clear();
