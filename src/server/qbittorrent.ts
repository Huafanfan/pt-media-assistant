...
export function sanitizeTorrent(raw: RawTorrent): TorrentSummary {
  const hash = textValue(
    raw.hash ?? raw.infohash_v1 ?? raw.infoHash ?? raw.infohash_v2,
  ).slice(0, 128);
...
export function sanitizeTorrents(value: unknown): TorrentSummary[] {
  ...
  return value
    .filter((item): item is RawTorrent =>
...
export function normalizeTitle(title: string): string {
  return stripReleaseExtension(title)
    ...
    .replace(/\[[^\]]*\]|\([^)]*\)|\{[^}]*\}/gu, " ")
    .replace(
      /\b(?:2160p|1080p|720p|4k|uhd|x26[45]|h\.?26[45]|hevc|av1)\b/giu,
      " ",
    )
    .replace(/[^\p{L}\p{N}]+/gu, "")
    .trim();
...
export function hasDuplicate(
  ...
  if (releaseHashes.size > 0) {
    if (
      torrents.some((torrent) =>
        hashCandidates(torrent).some((hash) => releaseHashes.has(hash)),
      )
    ) {
      return true;
    }
  }
  const releaseTitle = textValue(
    release.title ?? release.name ?? fallbackTitle,
  );
  ...
  return torrents.some((torrent) => {
    ...
    return (
      normalizedTorrent.length > 0 && normalizedTorrent === normalizedRelease
    );
  });
}
...
export class QBittorrentClient {
  ...
  private async login(): Promise<void> {
    ...
    const { response, body: text } = await this.rawRequest(
      "/api/v2/auth/login",
      {
        method: "POST",
  ...
  public async version(): Promise<string> {
    return (await this.request("/api/v2/app/version", { method: "GET" }))
      .trim()
      .slice(0, 80);
  }

  public async webApiVersion(): Promise<string> {
    return (await this.request("/api/v2/app/webapiVersion", { method: "GET" }))
      .trim()
      .slice(0, 80);
  }

  public async listTorrents(): Promise<TorrentSummary[]> {
    const body = await this.request("/api/v2/torrents/info?filter=all", {
      method: "GET",
    });
    try {
   ...
   */
  private async preferredControlEndpoints(): Promise<{
    pause: string;
    resume: string;
  }> {
   ...
   */
  public async torrentAction(
    action: TorrentAction,
    hashes: string[],
  ): Promise<void> {
    const normalized = [
      ...new Set(hashes.map((hash) => hash.trim().toLowerCase())),
    ];
    if (normalized.length !== 1) throw new QBittorrentError();
    if (!/^[a-z0-9]{32,64}$/u.test(normalized[0]!))
      throw new QBittorrentError();
    const headers = { "Content-Type": "application/x-www-form-urlencoded" };
    if (action === "remove") {
      const body = new URLSearchParams({
        hashes: normalized.join("|"),
        deleteFiles: "false",
      });
      const result = await this.execute("/api/v2/torrents/delete", {
        method: "POST",
        headers,
        body: body.toString(),
      });
    ...
    const result = await this.execute(`/api/v2/torrents/${primary}`, {
      method: "POST",
      headers,
      body,
    });
      ...
      const alternate =
        action === "pause"
          ...
          : primary === "start"
            ? "resume"
            : "start";
      const fallback = await this.execute(`/api/v2/torrents/${alternate}`, {
        method: "POST",
        headers,
        body,
      });
  ...
  public async duplicateForRelease(
    release: RawTorrent | Record<string, unknown>,
    fallbackTitle?: string,
  ): Promise<boolean> {
    const body = await this.request("/api/v2/torrents/info?filter=all", {
