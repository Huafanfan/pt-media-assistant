...
async function makeApp(
  torrentAction: (action: string, hashes: string[]) => Promise<void> = vi.fn(
    async () => undefined,
  ),
) {
  const prowlarr = {
...
describe("torrent task actions route", () => {
  ...
  it("maps adapter failures to a sanitized upstream error", async () => {
    ...
    const response = await app.inject({
      ...
      url: "/api/torrents/actions",
      headers,
      payload: { action: "remove", hashes: [hash] },
    });
...
function qbFetch(version: string) {
  ...
  const fetchImpl = vi.fn(
    async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes("/api/v2/torrents/"))
...
describe("qBittorrent control endpoint compatibility", () => {
  it("uses stop/start on Web API 2.11+ and pause/resume on 4.6.x", async () => {
    ...
    const modernClient = new QBittorrentClient({
      baseUrl: "http://127.0.0.1:8080",
      fetchImpl: modern.fetchImpl,
    });
    await modernClient.torrentAction("pause", [hash]);
    expect(
      modern.calls.some((call) => call.url.endsWith("/api/v2/torrents/stop")),
    ).toBe(true);

    const legacy = qbFetch("2.9.3");
    const legacyClient = new QBittorrentClient({
      baseUrl: "http://127.0.0.1:8080",
      fetchImpl: legacy.fetchImpl,
    });
    await legacyClient.torrentAction("pause", [hash]);
    expect(
      legacy.calls.some((call) => call.url.endsWith("/api/v2/torrents/pause")),
    ).toBe(true);
  });
  ...
  it("falls back once when the reported version uses the other endpoint family", async () => {
    ...
    const fetchImpl = vi.fn(async (input: RequestInfo | URL) => {
      ...
      calls.push(url);
      if (url.endsWith("/api/v2/app/webapiVersion"))
        return new Response("2.9.3", { status: 200 });
    ...
    expect(
      calls.filter((url) => url.endsWith("/api/v2/torrents/stop")),
    ).toHaveLength(1);
  });
  ...
  it("removes tasks without deleting files and rejects non-hash input", async () => {
    ...
    expect(
      calls.find((call) => call.url.endsWith("/api/v2/torrents/delete"))?.body,
    ).toContain("deleteFiles=false");
    await expect(client.torrentAction("remove", ["all"])).rejects.toThrow();
    await expect(client.torrentAction("pause", [])).rejects.toThrow();
    await expect(
      client.torrentAction("pause", [hash, "b".repeat(40)]),
    ).rejects.toThrow();
  });
});
