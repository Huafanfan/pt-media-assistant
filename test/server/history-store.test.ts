...
describe("持久化历史存储", () => {
  it("persists seen entries and preferences across reloads", async () => {
    ...
    expect(reloaded.snapshot().seen).toEqual([
      { mediaId: "1293000", mediaType: "movie", title: "星际穿越", markedAt },
    ]);
  ...
  it("adds display entries for AI-marked seen IDs only when a title is known", () => {
    ...
    store.savePreferences(
      {
        ...defaultAssistantPreferences(),
        seenMediaIds: ["movie:42", "movie:43"],
      },
      new Map([
        ["movie:42", { mediaId: "42", mediaType: "movie", title: "已知电影" }],
      ]),
    );
    expect(store.snapshot().seen).toEqual([
      { mediaId: "42", mediaType: "movie", title: "已知电影", markedAt },
    ]);
...
describe("会话从持久化状态初始化", () => {
  it("seeds preferences and accepts persisted seen IDs in model patches", () => {
    ...
    expect(() =>
      updatePreferences(turn.conversation, { seenMediaIds: ["movie:7"] }),
    ).not.toThrow();
    expect(() =>
      updatePreferences(turn.conversation, { seenMediaIds: ["movie:404"] }),
    ).toThrow();
  });
...
describe("持久化正确性回归", () => {
  ...
  it("keeps the full durable seen set authoritative beyond the preference cap", () => {
    ...
    expect(
      filterCandidateForPreferences(oldest, store.preferences(), known),
    ).toBe(false);
  });
