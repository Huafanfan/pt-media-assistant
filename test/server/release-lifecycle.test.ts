...
describe("recommendation snapshot references", () => {
  it("keeps snapshot references for 24 hours but leaves ordinary searches at 15 minutes", async () => {
    ...
    const client = new ProwlarrClient({
      ...
      cache,
      fetchImpl: async () =>
        Response.json([{ title: "Test 2024 1080p", seeders: 5, size: 100 }]),
    });
    const ordinary = await client.search({ searchTerm: "Other" });
    const service = new DiscoveryService(
      { douban, prowlarr: client },
    ...
    expect((await service.getMediaReleases("movie", "1")).snapshotId).toBe(
      snapshot.snapshotId,
    );
  ...
  it("marks an evicted reference unactionable without secretly querying again", async () => {
    ...
    const fetchImpl = vi.fn(async () =>
      Response.json([{ title: "Test 2024", seeders: 5 }]),
    );
    const client = new ProwlarrClient({
      baseUrl: "http://example.invalid",
      cache,
  ...
  it("evicts raw references with snapshot LRU and explicit cache clearing", async () => {
    ...
    const client = new ProwlarrClient({
      ...
      fetchImpl: async () =>
        Response.json([
          { title: `Test ${++searchIndex}`, seeders: 5, size: 100 },
        ]),
    });
    const service = new DiscoveryService(
      {
  ...
  it("invalidates old references before an explicit refresh", async () => {
    ...
    const first = await service.getMediaReleases("movie", "1");
    const refreshed = await service.getMediaReleases("movie", "1", 10, {
      forceRefresh: true,
    });
  ...
  it("keeps runtime release responses on the browser-safe DTO allowlist", async () => {
    ...
    expect(response.releases[0]).toEqual(
      expect.objectContaining({ categories: ["Movies"] }),
    );
  ...
  it("checks actual-search budgets including alternate titles, and does not cache failures", async () => {
    ...
    const service = new DiscoveryService(
      {
        douban: {
          ...douban,
          getMedia: async () => ({ ...media, originalTitle: "Original" }),
        },
        prowlarr: { search },
      },
      { minIntervalMs: 0 },
    ...
    await expect(
      service.getMediaReleases("movie", "1", 10, { beforeSearch }),
  ...
  it("does not search after cancellation while queued", async () => {
    ...
    const search = vi
      ...
      .mockResolvedValue([]);
    const service = new DiscoveryService(
      { douban, prowlarr: { search } },
      { minIntervalMs: 0 },
    );
    const first = service.getMediaReleases("movie", "1");
    const controller = new AbortController();
    const second = service.getMediaReleases("movie", "2", 10, {
  ...
  it("lets a valid waiter adopt the budget gate when the first waiter cancels", async () => {
    ...
    const service = new DiscoveryService(
      {
        douban: {
          ...
          getMedia: async (itemId: string) => ({
            ...media,
            id: itemId,
            title: itemId === "blocker" ? "Blocker" : "Test",
          }),
        },
        prowlarr: { search },
      },
    ...
    await expect(blockerRequest).resolves.toMatchObject({
      status: "unavailable",
    });
  ...
  it("aborts the shared upstream work when every waiter cancels", async () => {
    ...
    const first = service.getMediaReleases("movie", "1", 10, {
      signal: firstController.signal,
    });
    const second = service.getMediaReleases("movie", "1", 10, {
...
describe("release field evidence", () => {
  it("distinguishes absent freeleech and numeric fields from confirmed values", () => {
    const unknown = sanitizeRelease(
      { title: "Movie 1080p HEVC", size: null, seeders: "" },
      "release-id",
    );
    expect(unknown.freeleechState).toBe("unknown");
    expect(unknown.evidence).toEqual({
      resolution: "title_inferred",
      codec: "title_inferred",
      size: "unknown",
      seeders: "unknown",
      season: "unknown",
    });
    const known = sanitizeRelease(
      {
        title: "Movie",
        resolution: "1080p",
        codec: "HEVC",
    ...
    expect(
      sanitizeRelease({ indexerFlags: ["Freeleech"] }, "release-id")
        .freeleechState,
    ).toBe("yes");
    expect(
      sanitizeRelease({ indexerFlags: ["非免费"] }, "release-id")
        .freeleechState,
    ).toBe("unknown");
    expect(
      sanitizeRelease({ indexerFlags: ["非Freeleech"] }, "release-id")
        .freeleechState,
    ).toBe("unknown");
  });
  ...
  it("infers a season only from explicit season tokens in the title", () => {
    ...
    expect(sanitizeRelease({ title: "Show.Season.3.1080p" }, "id").season).toBe(
      3,
    );
    expect(sanitizeRelease({ title: "Show 第三季 1080p" }, "id").season).toBe(
      3,
    );
    expect(sanitizeRelease({ title: "Show 第十二季 1080p" }, "id").season).toBe(
      12,
    );
    expect(
      sanitizeRelease({ title: "Show S02 1080p" }, "id").evidence?.season,
    ).toBe("title_inferred");
    expect(
      sanitizeRelease({ title: "Show 1080p HEVC" }, "id").season,
    ).toBeUndefined();
    expect(
      sanitizeRelease({ title: "Show 1080p HEVC" }, "id").evidence?.season,
    ).toBe("unknown");
    // Bare numbers and words ending in "s" are never guessed as a season.
    expect(
      sanitizeRelease({ title: "Se7en 1995 1080p" }, "id").season,
    ).toBeUndefined();
    expect(
      sanitizeRelease({ title: "The Class 10 1080p" }, "id").season,
    ).toBeUndefined();
  });
  ...
  it("does not treat unknown size as satisfying a hard max-size constraint", async () => {
    const client = new ProwlarrClient({
      baseUrl: "http://example.invalid",
      fetchImpl: async () =>
        Response.json([{ title: "Movie 1080p", seeders: 5 }]),
    });
  ...
  it("preserves caller cancellation as an abort error", async () => {
    ...
    const request = client.search({ searchTerm: "Movie" }, 20, {
      signal: controller.signal,
    });
    controller.abort();
